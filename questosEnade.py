...
st.session_state.setdefault("text_base_saved", False)
...
def chamar_llm(prompts, prov, mdl, temperature=0.7, max_tokens=2000):
    if prov.startswith("OpenAI"):
        ...
        return m.generate_content(prompt, generation_config=cfg).text
...
def search_articles(query, num=5):
    ...
    for g in soup.select("div.g")[:num]:
        a=g.find("a", href=True); h3=g.find("h3")
        if a and h3: results.append({"title":h3.get_text(),"url":a["href"]})
    return results
...
with st.container():
    ...
    opc = st.radio("Deseja inserir um texto-base?", ["Não, IA gera automaticamente","Sim, inserir texto-base"], horizontal=True)
        ...
        if not st.session_state.auto:
            with st.spinner("Gerando texto-base automaticamente..."):
                prompts = [
                    ...
                    {"role":"user","content":f"Gere ~3 frases para situação-problema ENADE em Área: {area}, Curso: {curso}, Assunto: {assunto}."}
                ]
                tb = chamar_llm(prompts, provedor, modelo, temperature=0.5, max_tokens=300)
                st.session_state.text_base = tb or ""
                st.session_state.auto = True
                st.session_state.text_base_saved = True
        st.success("Texto-base gerado!")
    else:
        st.session_state.auto = False
        st.session_state.text_base_saved = False
        modo = st.radio("Como fornecer?", ["Upload de PDF","Buscar artigos na internet"], horizontal=True)
...
# --- 3. EDITAR E SALVAR TEXTO-BASE + REFERÊNCIA ---
if st.session_state.text_base:
    st.header("3. Texto-Base e Referência")
    tb_edited = st.text_area("Edite o Texto-Base:", st.session_state.text_base, height=200)
    if not st.session_state.auto:
        if st.button("💾 Salvar Texto-Base"):
            st.session_state.text_base = tb_edited
            st.session_state.text_base_saved = True
            st.success("Texto-base salvo!")
    else:
        # automático já está salvo
        pass

    if st.session_state.text_base_saved and not st.session_state.auto:
        ...
        if autor and titulo and veiculo and data_pub:
            ...
            st.session_state.ref_final = f"{autor}. {titulo}. {veiculo}, {data_pub}. Disponível em: <{info.get('link','')}>. Acesso em: {acesso}."
            st.text_area("Referência ABNT:", st.session_state.ref_final, height=80)

# --- 4. GERAÇÃO DA QUESTÃO ---
if st.session_state.text_base_saved and (st.session_state.auto or st.session_state.ref_final):
    ...
    if gerar:
        with st.spinner("Gerando..."):
            sys_p = """
Você é docente especialista do INEP. Gere UMA questão ENADE em texto puro, formato:
...
{"":"" if st.session_state.auto else "REFERÊNCIA:\n" + st.session_state.ref_final}

Por favor, siga EXATAMENTE o formato acima.
"""
            out = chamar_llm([{"role":"system","content":sys_p},{"role":"user","content":usr_p}], provedor, modelo, temperature=0.5, max_tokens=1000)
            if out:
                st.session_state.questoes.append(out)
                st.success("Questão gerada!")

# --- 5. RESULTADOS, DOWNLOAD & SALVAR BANCO ---
